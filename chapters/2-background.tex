...
\newpage
\input{tables/deid-transformations}TODO\todo{Can I just import table like this (plagiarism/copyright)?}\todo{Do generalisations, such as going from city to province, fall under aggregate?}
...
\todo[inline]{Write paragraph about different attacker model: now participant instead of third party}.\\

\todo[inline]{Write about requirements for MPC, ie fairness etc}.\\

\todo[inline]{Write about some basic techniques, such as PSI and encrypted gates}

\subsubsection{Differential Privacy \& \textit{k}-Anonymity}
\todo[inline]{Write section: for diff privacy see work by Cynthia Dwork, also explain why we can't use this in the middleware (only one user's dataset -> cant use k-anonymity, etc)}
